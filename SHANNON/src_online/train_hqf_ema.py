...
import logging
from data.dataset_part import get_training_hqf, get_test_hqf
import time
import loss
...
if __name__ == '__main__':
    ...
    train_set = get_training_hqf()
    test_set = get_test_hqf()
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', factor=0.7, verbose=True, patience=3)
    ...
    def train(epoch):
        ...
        for iteration, batch in enumerate(training_loader, 1):
            ...
            loss_car = criterion(prediction, target) + criterion_gdl(prediction[1], target) + criterion_gdl(prediction[2], target)
