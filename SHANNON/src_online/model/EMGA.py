...
# from .ops.DCNv2_latest.dcn_v2 import DCNv2
...
class CAR(nn.Module):
    def __init__(self, nf):
        ...
        self.head = nn.Sequential(
            nn.Conv2d(96, nf, kernel_size=1, stride=1),
        ...
        rfa1 = []
        rfa1.append(ConvNorm(nf * 4, nf * 2))
        rfa1.append(self.lrelu)
        rfa1.append(ConvNorm(nf * 2, nf))
        self.rfa1 = nn.Sequential(*rfa1)
        self.ca1 = CALayer(nf)

        self.fusion4 = ERIR(4, nf)
        self.fusion5 = ERIR(4, nf)

        rfa2 = []
        rfa2.append(ConvNorm(nf * 4, nf * 2))
        rfa2.append(self.lrelu)
        rfa2.append(ConvNorm(nf * 2, nf))
        self.rfa2 = nn.Sequential(*rfa2)
        self.ca2 = CALayer(nf)
    ...
    def forward(self, x, info):
        ...
        # B, C+C+C+C, H, W
        feats3_ = torch.cat([feats_ori, feats1, feats2, feats3], 1)
        feats3_ = self.ca1(self.rfa1(feats3_)) # add rfa

        feats4 = self.fusion4(feats3_, info)
        feats5 = self.fusion5(feats4, info)

        feats = torch.cat([feats_ori, feats3_, feats4, feats5], 1)
        feats = self.ca2(self.rfa2(feats)) # add rfa
