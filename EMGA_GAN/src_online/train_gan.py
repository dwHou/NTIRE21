...
if __name__ == '__main__':
    ...
    netD.load_state_dict(torch.load('./checkpoint/D_e19_0.096.pth'))

    criterion_g = loss.Loss(opt)
    ...
    optimizer_netD = optim.Adam(netD.parameters(), lr=opt.lr * 0.06)
    # lpips 不降，则减小学习率
    scheduler_netG = optim.lr_scheduler.ReduceLROnPlateau(optimizer_netG, 'min', factor=0.8, verbose=True, patience=6)
    ...
    def visual(lpips, epoch=0):
        img_list = fnmatch.filter(os.listdir("./visual/inputs/006"), '*.png')
        ...
        with torch.no_grad():
            list_mf = []
            for i in img_list:
                img_path = os.path.join("./visual/inputs/006", i)
            ...
            info_path = os.path.join("./visual/inputs/", "49.tuLayer.png")
    ...
    for epoch in range(1, opt.nEpochs + 1):
        ...
        if lr_g != optimizer_netG.param_groups[0]['lr']:
            lr_g = optimizer_netG.param_groups[0]['lr']
            logging.info('netG reducing lr of group 0 to {:.3e}'.format(lr_g))

        if lr_d != optimizer_netD.param_groups[0]['lr']:
            lr_d = optimizer_netD.param_groups[0]['lr']
            logging.info('netD reducing lr of group 0 to {:.3e}'.format(lr_d))

        if lpips <= best_lpips:
            best_lpips = lpips
            # model_best_path = os.path.join('.', 'checkpoint', f'G_e{epoch}_{lpips:.3f}.pth')
            logging.info(f'===> save the best model: reach {best_lpips:.3f} LPIPS')
            # torch.save(netG.state_dict(), model_best_path)

            # model_D = os.path.join('.', 'checkpoint', f'D_e{epoch}_{lpips:.3f}.pth')
