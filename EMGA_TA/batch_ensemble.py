...
def inference(gpu_idx, dir_idx):
    ...
    output_video = os.path.join(root, 'EMGA_out_ensemble', dir_idx)
    print(input_video)

    cmd = f"CUDA_VISIBLE_DEVICES={gpu_idx},{gpu_idx + 1} python3 src_online/EMGA_solver.py --cuda --ensemble --input_video {input_video} --output_video {output_video} --input_info {input_info}"
    print(cmd)
    run_cmd(cmd)
    # os.system(cmd)
...
for idx in allvideos:
    ...
    gpu_idx = m_idx
    print(f'=================> video {idx} using gpu idx {gpu_idx}, {gpu_idx + 1}')
