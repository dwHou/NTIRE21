...
def inference(gpu_idx, dir_idx):
    ...
    print(input_video)
    cmd = f"CUDA_VISIBLE_DEVICES={gpu_idx},{gpu_idx+1},{gpu_idx+2},{gpu_idx+3} python3 src_online/EMGA_solver.py --cuda --input_video {input_video} --output_video {output_video} --input_info {input_info}"
    print(cmd)
    run_cmd(cmd)
    # os.system(cmd)
