...
def inference(gpu_idx, dir_idx):
    ...
    print(input_video)
    cmd = f"CUDA_VISIBLE_DEVICES={gpu_idx},{gpu_idx+2} python3 src_online/ntire_solver.py --cuda --ensemble --input_video {input_video} --output_video {output_video} --input_info {input_info}"
    print(cmd)
    run_cmd(cmd)
    # os.system(cmd)
...
allvideos = [1000 + i for i in range(16, 21)]
threads = 2
pool = Pool(threads)
