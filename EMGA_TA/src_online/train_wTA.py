...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename='./LOG/' + 'TAVersion' + '.log', level=logging.INFO)
    ...
    for epoch in range(1, opt.nEpochs + 1):
        if epoch % 10 == 0:
            w = np.sum([np.array(w), np.array([-0.04, -0.02, 0, 0.02, 0.04])], axis=0)
            w = w.tolist()
        ...
        if psnr > best_psnr:
            best_psnr = psnr
            model_path = os.path.join('.', 'experiment', 'MwTA_{:.2f}dB_{:.2f}.pth'.format(psnr, var))
